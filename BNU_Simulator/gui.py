...
#窗口和容器都调宽了
        ...
        self.main_container.place(relx=0.5, rely=0.5, anchor='center', width=1200, height=800)  
        self.setup_ui()
        self.update_status()
...
#调整框架宽度和标签间距，使上方属性信息正常显示
        style.configure('Custom.TLabelframe', background='#F1F1F1', bordercolor='#2B428B', borderwidth=1, labelmargins=(8, 4, 0, 0))
        style.configure('Custom.TLabelframe.Label', background='#F1F1F1', font=('Simhei', 10))
##
        style.configure('TButton', background='#4a90d9', foreground='#333333', font=('Simhei', 12))
        style.map('TButton', background=[('active', '#2B428B')])
        style.configure("Status.TLabel", background="#4871A1", foreground="#F5FFFA", font=("Simhei", 12), padding=5)
        ...
        # 中间操作区
        self.action_frame = ttk.LabelFrame(main_frame, text="操作", padding=5, style='Custom.TLabelframe')
...
##
    ...
    def create_attribute_bars(self):
        ...
        for attr in attributes:
            ...
            # 使用SimSun字体
            label = ttk.Label(frame, text=f"{attr}: ", width=5, anchor="e", font=("SimSun", 12))
...
#适当调大字体和进度条长度           
        # 必修课进度
        ttk.Label(self.credits_frame, text="必修课:", font=("Simhei", 12)).grid(row=0, column=0, sticky="w", padx=5)
        ...
        # 专业选修I进度
        ttk.Label(self.credits_frame, text="专业选修I:", font=("Simhei", 12)).grid(row=0, column=3, sticky="w", padx=5)
        ...
        # 专业选修II进度
        ttk.Label(self.credits_frame, text="专业选修II:", font=("Simhei", 12)).grid(row=0, column=6, sticky="w", padx=5)
...
##
    ...
    def show_course_selection(self):
        ...
        # 使用Simhei字体
        ttk.Label(self.action_frame, text="选择本学年课程:", font=("Simhei", 14, "bold")).pack(anchor="w", pady=10)

        loading_label = ttk.Label(self.action_frame, text="加载课程中...", font=("Simhei", 14))
...
#调整字体和宽度
            ttk.Label(frame, text=course['name'], width=32, anchor="w", font=("KaiTi", 10)).pack(side=tk.LEFT)
            ...
            type_label = ttk.Label(frame, text=course['type'], font=("KaiTi", 10),width=10,
                                  foreground=type_color.get(course['type'], "black"))
            type_label.pack(side=tk.LEFT, padx=10)

            ttk.Label(frame, text=f"模块: {course['module']}", font=("KaiTi", 10),width=25).pack(side=tk.LEFT, padx=10)
            years = ', '.join(map(str, course['year_available']))
            ttk.Label(frame, text=f"开课学年: {years}", width=24, font=("KaiTi", 10)).pack(side=tk.LEFT, padx=5)
            ttk.Label(frame, text=f"学分: {course['credit']}", width=13, font=("KaiTi", 10)).pack(side=tk.LEFT, padx=5)
            ttk.Label(frame, text=f"压力: {course['pressure']}", width=13, font=("KaiTi", 10)).pack(side=tk.LEFT, padx=5)
